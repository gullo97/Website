...
st.write('The Harmonic Mapper model is able to learn a detailed vulnerability map: this allows it to assign the correct portion of vulnerability to the coordinates of a building and is thus the better choice for A-Posteriori vulnerability estimation. This capability is critical because we want the model to be able to distinguish the intrinsic vulnerability of building features from their position relative to an earthquake, thus using the position as a bias for prediction. The better the model is at interpolating a map (and maintaning satisfactory F1 score on validation), the more confident we are in its ability to evaluate intrinsic vulnerability of specific features.')
